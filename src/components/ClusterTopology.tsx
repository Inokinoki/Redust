...
export function ClusterTopology({ variant = "modal", isOpen = true, onClose }: ClusterTopologyProps) {
  ...
  const content = (
    <div className="h-full overflow-auto p-4">
      {error && (
        <div className="mb-4 rounded-lg border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950 p-4 text-red-600 dark:text-red-400">
          {error}
        </div>
      )}

      {clusterInfo && !clusterInfo.cluster_enabled && (
        <div className="mb-4 rounded-lg border border-yellow-200 dark:border-yellow-800 bg-yellow-50 dark:bg-yellow-950 p-4 text-yellow-600 dark:text-yellow-400">
      ...
      {clusterInfo && clusterInfo.cluster_enabled && (
        <div className="space-y-6">
          <div className="grid grid-cols-3 gap-4">
            <Card>
              <CardHeader>
                <CardTitle className="text-sm text-zinc-500 dark:text-zinc-400">Cluster State</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-sm text-zinc-500 dark:text-zinc-400">Known Nodes</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{clusterInfo.known_nodes}</div>
              </CardContent>
            </Card>

            <Card>
              <CardHeader>
                <CardTitle className="text-sm text-zinc-500 dark:text-zinc-400">Cluster Status</CardTitle>
              </CardHeader>
              <CardContent>
                    ...
                    <div className="space-y-1 text-sm">
                      ...
                      {node.master_id && (
                        <div className="flex justify-between">
                          ...
                          <span className="text-zinc-700 dark:text-zinc-300">{node.master_id}</span>
                        </div>
                      )}
    ...
    return (
      <div className="h-full">
        <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 px-4 py-2">
          <h3 className="text-sm font-semibold">Cluster Topology</h3>
          <Button onClick={loadClusterInfo} disabled={loading} variant="outline" size="sm">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="relative z-50 max-h-[90vh] w-full max-w-6xl overflow-y-auto rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-6 shadow-xl">
